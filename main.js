...
const list = document.querySelector('#list'); // Se refiere al elemento visual del DOM
const input = document.querySelector('#input');
const add_btn = document.querySelector('#add_btn');
const uncheck = 'bi-circle';
const check = 'bi-record-circle';
const crossed_off = 'crossed_off';
...
function addTask(task, id, done = false, del = false) {
    ...
    const taskCard = `<li class="task-item" id="${id}">
      <i class="bi ${complete}" onclick="taskDone(this)"></i>
      <p class="text ${LINE}">${task}</p>
      <i class="bi bi-x" onclick="taskDeleted(this)"></i>
    </li>`;
